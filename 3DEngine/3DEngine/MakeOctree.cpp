...
// Maximum amount of triangle in tris in a leaf outside the recursion limit
...
{
	Octree* MakeOctree(
		const Tris& tris, const Box3D& box, int recursionDepth)
	{
		assert(!tris.empty());
		...
		for (int i = 0; i < 8; i++)
		{
			Box3D childBox = box.MakeOctreeChild(i);
